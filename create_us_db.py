...
# ==================== 설정 ====================
TICKER_LIST_PATH = "UsStockCodeList.json"
DB_PATH = "us.db"
...
def update_database(date_str: str, webhook_url: str = None):
    ...
    with db:
        ...
        if db_exists and db.table_exists():
            ...
            if existing_count > 0:
                skip_message = f"⚠️ **미국 주식 DB 업데이트 스킵**\n📅 날짜: {date_str}\n💾 이미 존재하는 데이터: {existing_count}개"
        ...
        if not new_data_list:
            error_message = f"❌ **미국 주식 DB 업데이트 실패**\n📅 날짜: {date_str}\n⚠️ 수집된 데이터가 없습니다."
...
사용 예제:
  ...
  python create_us_db.py --date 2026-01-03    # 특정 날짜로 업데이트
    ...
    parser.add_argument(
        '--date',
        help='업데이트할 날짜 (YYYY-MM-DD, 기본값: 어제)',
